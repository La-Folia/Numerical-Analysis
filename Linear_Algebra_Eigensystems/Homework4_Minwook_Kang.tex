...
이를 확인하면, 우리가 구할려고 했던 Root Finding Solution과 전혀 다른 값이 나온다. 그런, 그래프 모양을 잘 살펴보면, 사실 실제 근과 비슷하게 만들어진 것을 볼 수 있는데, 이는 우리가 정한 두 경계값 사이에서 함수와 비슷한 모델을 만들려고 하니 만들어진 결과라고 볼 수 있다.  따라서 만약 우리가 원하는 해에 근사한 경계값만 알게 된다면, 더 정확도가 높은 값을 유추할 수 있다. 예를 들어, x = 0 을 고정으로 두고, 만약 y값이 실제의 경계값과 닮아가는 그래프를 다시 그리게 되면,
...
\begin{figure}[!ht]
  ...
  \caption{Two-point BVPs}
\end{figure}
이렇게 우리가 구하고자 하는 값과 잘 맞아 떨어지는 것을 확인해볼 수 있다. 
